    ...
    <div class="allMuhtar">
        <div class="content">
            ...
            <div class="muhtarArea">
                <?php
                if($il != "" || $ilce != "" || $mahalle != "" ) {
                    $sorgu4 = $baglanti->prepare("SELECT  m.id, m.muhName, m.mahalle_key, m.image, mh.mahalle_title, mh.mahalle_ilcekey, m.aktif  FROM muhtarlar m INNER JOIN mahalle mh WHERE m.mahalle_key =:mahkey AND mh.mahalle_key =:mahkey2 and m.aktif =1 and (m.status = 0 or m.status = 2 ) and m.shows = 1 ORDER BY m.id DESC");
                    $sorgu4->execute([
                    ...
                    $sorgu5 = $baglanti->prepare("SELECT m.id, m.muhName, m.image, mh.mahalle_title, mh.mahalle_ilcekey FROM muhtarlar m INNER JOIN mahalle mh WHERE m.mahalle_key = mh.mahalle_key AND m.aktif = 1 and (m.status = 0 or m.status = 2) and m.shows = 1 ORDER BY m.id DESC");
                    $sorgu5->execute();
