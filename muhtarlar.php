...
?>
    <div class="homeSlide sloganSlide">
        <div class="content">
            <div class="bigText">
                <h1>
                    Mevcut Muhtarları<br>görmek için<br>
    ...
    <div class="mahSelector scrollby" id="mahSelector">
        <div class="content">
            <form id="selectorMuh2">
                <span>Şehir seçiniz</span>
                <select class="sehir" id="sehir" name="sehir">
                    <option value="">Şehir Seçiniz</option>
                    <?php
                    $sorgu3=$baglanti->prepare("SELECT sehir_title, sehir_key FROM sehir ORDER BY sehir_title");
                    $sorgu3->execute();
                ...
                <select name="mahalle" id="mahalle" class="mahalle">
                    ...
                    <?php if($_GET) {
                        $sorgu2 = $baglanti->prepare("SELECT mahalle_title, mahalle_key FROM mahalle WHERE mahalle_ilcekey =:ilcekodu ORDER BY mahalle_title");
    ...
    <div class="allMuhtar">
        <div class="content">
            ...
            <div class="muhtarArea">
                <?php
                if($il != "" || $ilce != "" || $mahalle != "" ) {
                    $sorgu4 = $baglanti->prepare("SELECT  m.id, m.muhName, m.mahalle_key, m.image, mh.mahalle_title, mh.mahalle_ilcekey, m.aktif  FROM muhtarlar m INNER JOIN mahalle mh WHERE m.mahalle_key =:mahkey AND mh.mahalle_key =:mahkey2 and m.aktif =1 and (m.status = 1 or m.status = 2 ) and m.shows = 1 ORDER BY m.id DESC");
                    $sorgu4->execute([
                    ...
                    :0 = sadece aday
                    :1 = sadece muhtar
                    :2 = hem aday hem muhtar*/

                    $sorgu5 = $baglanti->prepare("SELECT m.id, m.muhName, m.image, mh.mahalle_title, mh.mahalle_ilcekey FROM muhtarlar m INNER JOIN mahalle mh WHERE m.mahalle_key = mh.mahalle_key AND m.aktif = 1 and (m.status = 1 or m.status = 2) and m.shows = 1 ORDER BY m.id DESC");
                    $sorgu5->execute();
